...
export default function Dashboard() {
  ...
  if (error)
    return (
      <main className="page-shell text-red-400">
        Unable to load dashboard. Check that the API is running.
      </main>
    );
  return (
    <main className="page-shell">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link
          ...
          className="rounded-xl bg-[#F4A91C] px-5 py-3 font-bold text-black"
        >
          + New Analysis
        </Link>
      </div>
      <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard label="High Risk" value={count("HIGH_RISK")} />
        <StatCard label="Likely Bots" value={count("LIKELY_BOT")} />
      </div>
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-800 bg-[#111827] p-4 sm:p-5">
          ...
          <p className="mt-3 text-slate-400">
            Average across your available analyses.
          </p>
        </div>
      </div>
    </main>
